...
function register(e) {
  ...
  const existingRow = data.slice(1).findIndex(row =>
    row[1] === p.name && String(row[3]) === String(p.phone) && row[8] !== '취소'
  );
...
function cancelRegistration(e) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (data[i][1] === name && phone.slice(-4) === String(phone4) && data[i][8] !== '취소') {
...
function updateStatus(e) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][1] === name && String(data[i][3]) === String(phone)) { // Name & Phone match
